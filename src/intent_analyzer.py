...
class IntentAnalyzer:
    ...
    def _extract_location(self, text: str) -> Optional[str]:
        ...
        time_words = ['сегодня', 'завтра', 'послезавтра', 'на', 'в',
        ...
        # Ищем слова с заглавной буквы в ОРИГИНАЛЬНОМ тексте
        words = text.split()

        for i, word in enumerate(words):
            # Пропускаем первые слова если они предлоги
            if i == 0 and word.lower() in ['в', 'на', 'для']:
                continue
            ...
            if word and word[0].isupper():
                ...
                if lower_word not in time_words:
                    # Берем только первое найденное слово с заглавной
                    return word.strip(',.!?')

        # Если не нашли - возможно город написан с маленькой буквы
        # Ищем любые существительные (слова длиннее 3 букв)
        for word in words:
            if len(word) >= 4 and word.isalpha():
                # Пропускаем предлоги и временные слова
                if word.lower() not in ['погода', 'какая', 'какой', 'будет', 'какое']:
                    return word.title()
