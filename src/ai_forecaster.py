import requests
import re
from datetime import datetime
from typing import Dict, Any, List, Tuple, Optional
import traceback

from .config import config


class AIForecaster:
    """Класс для работы с Groq API"""

    SYSTEM_PROMPT = """
    Ты — эксперт по рыбалке, профессиональный рыбак и ихтиолог.
    Ты работаешь внутри Telegram-бота и анализируешь прогноз погоды на 5 дней, полученный по API, для выбранной пользователем локации.

    Ты учитываешь:
    — температуру воздуха;
    — атмосферное давление;
    — ветер (скорость и направление);
    — осадки;
    — облачность;
    — влажность;
    — температуру воды (если дана);
    — сезон года;
    — тип водоёма (если указан);
    — название водоёма (если указано).

    Твоя задача:
    Проанализировать влияние погоды и сезона на активность рыбы и выдать прогноз клёва.

    Разделяй прогноз на:
    — 🐟 Мирная рыба
    — 🦈 Хищная рыба

    Формат ответа:

    📍 Локация: <место>

    🗓 Сезон: <весна / лето / осень / зима>

    🎯 Общий прогноз на 5 дней:
    Краткое описание тенденции (улучшение / ухудшение / стабильный клёв)

    🐟 Мирная рыба:
    Прогноз клёва: X/10  
    Короткое объяснение факторов.

    🦈 Хищная рыба:
    Прогноз клёва: X/10  
    Короткое объяснение факторов.

    📊 Ключевые погодные факторы:
    - ...
    - ...

    🧰 Рекомендации:
    - какие снасти
    - какие приманки / насадки
    - время суток

    📍 Где искать рыбу:
    - типичные точки

    Если название водоёма или его тип не указаны — попроси пользователя добавить:
    «Уточните, пожалуйста, название и тип водоёма (река, озеро, водохранилище). Это повысит точность прогноза.»

    Если не указана температура воды — скажи, что расчёт выполнен по сезонной средней.

    Отвечай кратко, понятно, без длинных вступлений.
    Используй дружелюбный тон опытного рыбака-наставника.
    
    Ты используешь следующие правила поведения рыб:

    Мирная рыба:
    — Карп, карась, лещ, плотва лучше клюют при стабильной погоде, слабом ветре и тёплой воде.
    — Падение давления и резкие похолодания ухудшают клёв.
    — Утро и вечер — основное время питания летом.
    
    Хищная рыба:
    — Щука активна при пасмурной погоде, умеренном ветре, лёгком понижении давления.
    — Судак любит стабильное или высокое давление и активен ночью и на рассвете.
    — Окунь стабилен при спокойной погоде, реагирует на смену облачности.
    
    Сезонность:
    — Весна: активизация после нереста, лучший клёв при потеплении.
    — Лето: стабильный клёв утром и вечером, днём снижение.
    — Осень: пик активности хищника при похолодании воды.
    — Зима: снижение активности, локальные выходы на кормёжку.
    
    Если вид конкретной рыбы не указан — давай прогноз обобщённо для мирной и хищной группы.
    """

    def __init__(self):
        self.api_key = config.GROQ_API_KEY
        self.api_url = config.GROQ_API_URL
        self.model = "openai/gpt-oss-120b"  # Бесплатная модель Groq

    def _create_user_prompt(self, region: str, weather_data: List[Dict[str, Any]]) -> str:
        """Создание промпта для ИИ на основе погодных данных"""
        weather_info = []

        for i, day in enumerate(weather_data, 1):
            date = datetime.fromisoformat(day['date']).strftime('%d.%m.%Y')
            weather_info.append(
                f"День {i} ({date}):\n"
                f"- Температура: {day['temperature']:.1f}°C (ощущается как {day['feels_like']:.1f}°C)\n"
                f"- Давление: {day['pressure']} мм рт.ст.\n"
                f"- Влажность: {day['humidity']}%\n"
                f"- Ветер: {day['wind_speed']:.1f} м/с, направление: {day['wind_direction']}°\n"
                f"- Облачность: {day['cloudiness']}%\n"
                f"- Осадки: {day['precipitation']:.1f} мм\n"
                f"- Описание: {day['description']}\n"
            )

        user_prompt = (
            f"Регион: {region}\n"
            f"Количество дней: {len(weather_data)}\n"
            f"Текущая дата: {datetime.now().strftime('%d.%m.%Y %H:%M')}\n\n"
            f"ПОГОДНЫЕ ДАННЫЕ:\n{'-' * 40}\n"
            f"{''.join(weather_info)}\n"
            f"{'-' * 40}\n"
            f"Проанализируй эти данные и дай прогноз клева."
        )

        return user_prompt

    def _extract_forecast_quality(self, ai_response: str) -> Tuple[Optional[str], Optional[float]]:
        """Извлечение качества клева и уверенности из ответа ИИ"""
        try:
            quality_match = re.search(r'ОБЩАЯ ОЦЕНКА:\s*(\d)/5', ai_response, re.IGNORECASE)
            confidence_match = re.search(r'УВЕРЕННОСТЬ:\s*(\d+)%', ai_response, re.IGNORECASE)

            quality_map = {
                '5': 'Отличный',
                '4': 'Хороший',
                '3': 'Средний',
                '2': 'Слабый',
                '1': 'Отсутствует'
            }

            quality = None
            confidence = None

            if quality_match:
                score = quality_match.group(1)
                quality = quality_map.get(score, 'Средний')

            if confidence_match:
                confidence = float(confidence_match.group(1))

            return quality, confidence

        except Exception as e:
            print(f"❌ Ошибка извлечения качества прогноза: {e}")
            return None, None

    def get_forecast(self, region: str, weather_data: List[Dict[str, Any]]) -> Dict[str, Any]:
        """Получение прогноза клева от Groq API"""
        try:
            headers = {
                "Authorization": f"Bearer {self.api_key}",
                "Content-Type": "application/json"
            }

            payload = {
                "model": self.model,
                "messages": [
                    {"role": "system", "content": self.SYSTEM_PROMPT},
                    {"role": "user", "content": self._create_user_prompt(region, weather_data)}
                ],
                "temperature": 0.3,
                "max_tokens": 1500
            }

            print(f"🤖 Запрашиваем прогноз у Groq для {region}")

            response = requests.post(
                self.api_url,
                headers=headers,
                json=payload,
                timeout=80
            )

            if response.status_code != 200:
                print(f"❌ Ошибка Groq API: {response.status_code}")
                print(f"Ответ: {response.text[:200]}")
                return self._get_fallback_forecast(region, weather_data)

            result = response.json()
            ai_response = result['choices'][0]['message']['content']

            quality, confidence = self._extract_forecast_quality(ai_response)

            print(f"✅ Получен прогноз от Groq. Качество: {quality}, Уверенность: {confidence}%")

            return {
                "ai_response": ai_response,
                "quality": quality,
                "confidence": confidence or 85.0
            }

        except requests.exceptions.RequestException as e:
            print(f"❌ Ошибка запроса к Groq API: {e}")
            return self._get_fallback_forecast(region, weather_data)
        except Exception as e:
            print(f"❌ Неожиданная ошибка при работе с Groq: {e}")
            traceback.print_exc()
            return self._get_fallback_forecast(region, weather_data)

    def _get_fallback_forecast(self, region: str, weather_data: List[Dict[str, Any]]) -> Dict[str, Any]:
        """Резервный алгоритм на случай недоступности Groq API"""
        print(f"⚠️ Используем резервный алгоритм для {region}")

        scores = []

        for day in weather_data:
            score = 3.0

            if 759 <= day['pressure'] <= 763:
                score += 0.5
            elif day['pressure'] < 750:
                score -= 1.0
            elif day['pressure'] > 766:
                score -= 0.5

            if 15 <= day['temperature'] <= 25:
                score += 0.5
            elif day['temperature'] < 5 or day['temperature'] > 30:
                score -= 1.0

            if day['wind_speed'] > 6:
                score -= 0.5

            if 0.1 <= day['precipitation'] <= 5:
                score += 0.3
            elif day['precipitation'] > 10:
                score -= 0.5

            scores.append(max(1.0, min(5.0, score)))

        avg_score = sum(scores) / len(scores)

        if avg_score >= 4.5:
            quality = "Отличный"
        elif avg_score >= 3.5:
            quality = "Хороший"
        elif avg_score >= 2.5:
            quality = "Средний"
        elif avg_score >= 1.5:
            quality = "Слабый"
        else:
            quality = "Отсутствует"

        response_lines = [
            f"Название региона: {region}",
            "",
            f"📊 ОБЩАЯ ОЦЕНКА: {avg_score:.1f}/5 - {quality}",
            "🎯 УВЕРЕННОСТЬ: 70%",
            "",
            "📅 ПРОГНОЗ ПО ДНЯМ:"
        ]

        for i, (day, score) in enumerate(zip(weather_data, scores), 1):
            date = datetime.fromisoformat(day['date']).strftime('%d.%m')
            response_lines.append(
                f"{i}. {date}: {score:.1f}/5 - "
                f"Темп: {day['temperature']:.1f}°C, Давление: {day['pressure']} мм рт.ст."
            )

        response_lines.extend([
            "",
            "⚡ КЛЮЧЕВЫЕ ФАКТОРЫ:",
            "• Использован резервный алгоритм (Groq API недоступен)",
            "• Учитывались: давление, температура, ветер, осадки",
            "",
            "💡 РЕКОМЕНДАЦИИ:",
            "• Система Groq API временно недоступна",
            "• Прогноз рассчитан по упрощенному алгоритму",
            "",
            "⚠️ ВНИМАНИЕ: Это автоматический прогноз без анализа ИИ"
        ])

        ai_response = "\n".join(response_lines)

        return {
            "ai_response": ai_response,
            "quality": quality,
            "confidence": 70.0
        }


# Глобальный экземпляр ИИ-прогнозировщика
ai_forecaster = AIForecaster()
