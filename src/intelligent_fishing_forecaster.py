...
class IntelligentFishingForecaster:
    ...
    def _create_fishing_prompt(self, weather_data: Dict, user_query: str) -> str:
        ...
        ФОРМАТ ОТВЕТА (без таблиц и Markdown-таблиц, только списки):
        🎣 Прогноз клева: <локация>
        📅 Период: <даты или диапазон>
        ✅ Общая оценка: X/10 — коротко почему (1-2 предложения)

        🐟 Мирная рыба:
        - Активность: X/10
        - Где искать: 1-2 пункта
        - Насадки/приманки: 1-3 пункта
        ...
        ⏰ Лучшее время:
        - 1-2 интервала (утро/день/вечер/ночь)

        🧰 Рекомендации:
        - 3-5 коротких советов

        🌙 Луна и давление:
        - 1-2 предложения о влиянии

        Отвечай на русском языке, будь конкретным и практичным, без лишней воды.
        """
